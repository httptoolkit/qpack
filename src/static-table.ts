...
/**
 * The QPACK static table, as defined in RFC 9204 appendix A. This copy was
 * extracted mechanically from the RFC text and cross-checked against
 * ls-qpack's table (see test/fixtures/static-table.json and the static
 * table tests).
 */
export const STATIC_TABLE: readonly HeaderField[] = [
...
/** Exact name+value matches: 'name\0value' -> lowest static table index */
export const STATIC_EXACT_MATCHES = new Map<string, number>();

/** Name-only matches: name -> lowest static table index with that name */
export const STATIC_NAME_MATCHES = new Map<string, number>();

for (let i = 0; i < STATIC_TABLE.length; i++) {
    const { name, value } = STATIC_TABLE[i]!;
    const exactKey = name + '\0' + value;
    if (!STATIC_EXACT_MATCHES.has(exactKey)) STATIC_EXACT_MATCHES.set(exactKey, i);
    if (!STATIC_NAME_MATCHES.has(name)) STATIC_NAME_MATCHES.set(name, i);
}
